import type { AnyAction, ActionCreator, ExtractAction } from './types';
import { getType } from './getType';

function castArray<TValue>(value: TValue | ReadonlyArray<TValue>): TValue[] {
...
): <TSource extends AnyAction, TSink extends TSource = ExtractAction<TSource, TKey>>(
  action: TSource,
) => action is TSink;
...
export function isOfType<TSource extends AnyAction, TKey extends string | AnyAction | ActionCreator<AnyAction>>(
  ...
  const types = castArray<string | AnyAction | ActionCreator<AnyAction>>(keys).map((key) =>
    typeof key === 'string' ? key : getType(key),
  );

  const assertFn = (_action: TSource) => types.includes(_action.type);
